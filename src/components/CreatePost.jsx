...
const CreatePost = (props) => {
    ...
    //     const token = await createPost(title, price, description, location)
    //    console.log(token)
    ...
    return (
        <div className="createBox">
          ...
          <input id="postTitle" type="text" required value={title} onChange={(event) => setPostTitle(event.target.value)}/>
          <label htmlFor="price">Price:</label>
          <input id="postPrice" type="text" required value={price} onChange={(event) => setPostPrice(event.target.value)}/>
          <label htmlFor="description">Description:</label>
          <textarea id="postDescription" required value={description} onChange={(event) => setPostDescription(event.target.value)}></textarea>
          <label htmlFor="location">Location:</label>
          <input id="postLocation" type="text" required value={location} onChange={(event) => setPostLocation(event.target.value)}/>
          { !isLoading && <button type="submit">Create Post</button>}
          {isLoading && <button disabled type="submit">Adding Post...</button>
          }
